                     ...
                     <div className="flex-1 bg-gradient-to-br from-slate-50 to-slate-100 relative p-8 pt-24">
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-full">
                              ...
                              <p className="text-slate-500">Loading {selectedCity} data...</p>
                            </div>
                        ) : scenarioData ? (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              ...
                              {activeScenario === 'Agri' && (
                                <>
                                  <DataCard label="Soil Moisture" value={`${(scenarioData.soilMoisture * 100).toFixed(1)}%`} color="emerald" icon={Sprout} />
                                  <DataCard label="Soil Temp" value={`${scenarioData.soilTemp?.toFixed(1)}°C`} color="amber" icon={ThermometerSun} />
                                  <DataCard label="Precipitation" value={`${scenarioData.rain?.toFixed(1)} mm`} color="blue" icon={Activity} />
                                  <DataCard label="Air Temp" value={`${scenarioData.realTemp?.toFixed(1)}°C`} color="rose" icon={ThermometerSun} />
                                </>
                              )}
                              {activeScenario === 'Energy' && (
                                <>
                                  <DataCard label="Solar Radiation" value={`${scenarioData.solarRad?.toFixed(1)} MJ/m²`} color="amber" icon={Zap} />
                                  <DataCard label="Cloud Cover" value={`${scenarioData.cloud?.toFixed(0)}%`} color="slate" icon={Activity} />
                                  <DataCard label="Wind Speed" value={`${scenarioData.wind?.toFixed(1)} km/h`} color="blue" icon={Activity} />
                                  <DataCard label="Air Temp" value={`${scenarioData.realTemp?.toFixed(1)}°C`} color="rose" icon={ThermometerSun} />
                                </>
                              )}
                              {activeScenario === 'Supply' && (
                                <>
                                  <DataCard label="Wind Gusts" value={`${scenarioData.gust?.toFixed(1)} km/h`} color="purple" icon={Truck} />
                                  <DataCard label="Avg Wind" value={`${scenarioData.wind?.toFixed(1)} km/h`} color="blue" icon={Activity} />
                                  <DataCard label="Snowfall" value={`${scenarioData.snow?.toFixed(1)} cm`} color="cyan" icon={Activity} />
                                  <DataCard label="Temperature" value={`${scenarioData.realTemp?.toFixed(1)}°C`} color="rose" icon={ThermometerSun} />
                                </>
                              )}
                              {activeScenario === 'Health' && (
                                <>
                                  <DataCard label="Feels Like" value={`${scenarioData.feelTemp?.toFixed(1)}°C`} color="rose" icon={ThermometerSun} />
                                  <DataCard label="Actual Temp" value={`${scenarioData.realTemp?.toFixed(1)}°C`} color="amber" icon={ThermometerSun} />
                                  <DataCard label="Precipitation" value={`${scenarioData.rain?.toFixed(1)} mm`} color="blue" icon={Activity} />
                                  <DataCard label="UV Proxy" value={`${scenarioData.solarRad?.toFixed(0)} MJ`} color="yellow" icon={Zap} />
                                </>
                              )}
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center h-full">
                                ...
                                <h3 className="text-2xl font-bold text-slate-700">Waiting for API Data</h3>
                                <p className="text-slate-400 max-w-md mt-3">
                                  Connecting to Open-Meteo API for <strong>{selectedCity}</strong>...
                                </p>
                            </div>
                        )}
                     </div>
                 ...
                 <div>
                      ...
                      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-2xl border border-indigo-200 mb-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-indigo-600 uppercase">AI-Driven Insight</span>
                          </div>
                        ...
                        {aiBriefing.keyPoints && aiBriefing.keyPoints.length > 0 && (
                          <div className="bg-white/60 p-4 rounded-xl mb-4">
                            <p className="text-xs font-bold text-slate-500 uppercase mb-2">Key Points</p>
                        ...
                        {aiBriefing.policyRelevance && (
                          <div className="bg-white/60 p-4 rounded-xl">
                            <p className="text-xs font-bold text-slate-500 uppercase mb-2">Policy Document Reference</p>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className={`p-6 rounded-2xl border shadow-sm transition-all h-full ${
                         ...
                         <div className="text-xs font-bold text-slate-400 uppercase mb-1">Current Signal</div>
                      ...
                      >
                         ...
                         <div className="text-lg font-bold text-slate-800 mb-2 line-clamp-2">
                             {aiBriefing.policyMatch?.title || "Loading Policy DB..."}
                         </div>
                      ...
                      <div className="bg-green-50 p-6 rounded-2xl border border-green-200 shadow-sm relative overflow-hidden group">
                         <div className="relative z-10">
                            <div className="flex items-center gap-2 mb-4">
                               ...
                               <span className="text-xs font-bold text-green-700 uppercase">Recommendation</span>
                            </div>
                 ...
                 <div className="mt-8">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-slate-800">Related Policy Documents</h3>
                          <p className="text-sm text-slate-500">{currentCity.country} - {activeScenario} Scenario ({aiBriefing.relatedPolicies.length} documents)</p>
                        </div>
                      </div>
                      <button 
             ...
             <div className="max-w-7xl mx-auto animate-in fade-in">
                <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
                  <div>
                    ...
                    <p className="text-slate-500 mt-1">
                      {policyLoading ? 'Loading documents...' : `Showing ${contextPolicies.length} documents from backend`}
                    </p>
                  </div>
                ...
                {policyLoading ? (
                  <div className="text-center py-16">
                    ...
                    <p className="text-slate-500">Loading documents from server...</p>
                  </div>
                ) : contextPolicies.length === 0 ? (
                  <div className="text-center py-16 bg-slate-50 rounded-2xl">
                    <FileText className="w-12 h-12 text-slate-300 mx-auto mb-4"/>
                    <p className="text-slate-500">No documents found. Make sure the backend server is running.</p>
                  </div>
                ) : (
                  ...
                  {contextPolicies.map(doc => (
                    <div key={doc.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all flex flex-col h-full">
                       ...
                       <div className="mt-auto bg-amber-50/50 p-3 rounded-xl border border-amber-100 mb-4">
                          <p className="text-[10px] font-bold text-amber-700 mb-1 uppercase tracking-wide">File Info</p>
               ...
               <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in">
                   <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                       <div>
                           ...
                           <p className="text-xs text-slate-500">
                             Filtered view for: {activeScenario} Scenario • {selectedPeriod === '1W' ? 'Last 7 Days' : selectedPeriod === '1M' ? 'Last 30 Days' : selectedPeriod === '1Q' ? 'Last 90 Days' : 'Last 365 Days'} ({rawData.length} records)
                           </p>
                       </div>
                       <div className="flex items-center gap-3">
                   ...
                   {isLoading ? (
                     <div className="p-12 text-center text-slate-400">
                       ...
                       <p className="text-sm">Loading {selectedPeriod} data...</p>
                     </div>
                   ) : (
